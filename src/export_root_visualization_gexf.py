...
DEFAULT_OUTPUT = Path("outputs/root_visualization/gephi/root_visualization.gexf")


def hex_to_rgba(hex_color: str) -> dict[str, float]:
    """Convert a hex color like #C7D92A to Gephi viz color dict."""
...
def build_root_gexf_graph(nodes_df: pd.DataFrame, pair_df: pd.DataFrame) -> nx.Graph:
    """Build a root-only graph with Gephi viz size/color embedded."""
    graph = nx.Graph()
    ...
    for row in filtered_pairs.itertuples(index=False):
        graph.add_edge(
            row.source_node_id,
            row.target_node_id,
            weight=float(row.combined_similarity),
            text_similarity=float(row.text_similarity),
            co_citation_similarity=float(row.co_citation_similarity),
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Export a root-only GEXF with Gephi viz color and size."
    )
...
def main() -> None:
    ...
    graph = build_root_gexf_graph(nodes_df, pair_df)
